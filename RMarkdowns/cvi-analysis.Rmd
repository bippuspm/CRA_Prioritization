...
#EDCs
edcs_violin_cvi = ggplot(cvi_edcs, aes(x = chemical, y = CVI.Score, fill = chemical)) + geom_violin(trim = FALSE) + theme_bw() + ylab("CVI Score") + xlab("Chemical") + theme(axis.text.x = element_text(size = 10, angle = 45, hjust = 1), legend.position = "none") + stat_summary(fun = mean, geom = "point", shape = 20, size = 5, color = "black", fill = "gray") + geom_hline(yintercept = mean(cvi_edcs$CVI.Score), linetype = "dashed")  + scale_y_continuous(breaks = seq(0.2, 0.8, 0.1), limits = c(0.2,0.8)) + scale_fill_viridis(discrete = TRUE, option = "H", begin = 0.2) + scale_x_discrete(labels = c("Bisphenol A*", "Chromium", "Cyanide compounds", "Decabromodiphenyl oxide", "Lead", "Nonylphenol", "Styrene*"))
