...
```{r}
#cns = 52% (5,626 out of 10,750)
((length(cns[["trifd"]]) - length(unique(cns[["trifd"]]))) / length(unique(cns[["trifd"]])))*100
(length(cns[["trifd"]]) - length(unique(cns[["trifd"]])))
(length(unique(cns[["trifd"]])))

#carc = 42% (4,675 out of 11,005)
((length(carc[["trifd"]]) - length(unique(carc[["trifd"]]))) / length(unique(carc[["trifd"]])))*100
(length(carc[["trifd"]]) - length(unique(carc[["trifd"]])))
(length(unique(carc[["trifd"]])))

#cardioresp = 68% (7,914 out of 11,706)
((length(cardioresp[["trifd"]]) - length(unique(cardioresp[["trifd"]]))) / length(unique(cardioresp[["trifd"]])))*100
(length(cardioresp[["trifd"]]) - length(unique(cardioresp[["trifd"]])))
(length(unique(cardioresp[["trifd"]])))

#lkt = 43% (4,992 out of 11,486)
((length(lkt[["trifd"]]) - length(unique(lkt[["trifd"]]))) / length(unique(lkt[["trifd"]])))*100
(length(lkt[["trifd"]]) - length(unique(lkt[["trifd"]])))
(length(unique(lkt[["trifd"]])))

#devrep = 28% (2,905 out of 10,426)
((length(devrep[["trifd"]]) - length(unique(devrep[["trifd"]]))) / length(unique(devrep[["trifd"]])))*100
(length(devrep[["trifd"]]) - length(unique(devrep[["trifd"]])))
(length(unique(devrep[["trifd"]])))
```
...
carc_heat = ggplot(carc_co_long, aes(x = chemical1, y = chemical2, fill = value)) +
  ...
  geom_text(
    aes(
      ...
      color = ifelse(value > max(carc_co_long$value, na.rm = TRUE) * 0.5,
                     "white", "black")
    ),
    size = 3
  ) +
  scale_color_identity() +   # important so ggplot uses your colors directly
  scale_fill_gradientn(
    colors = brewer.pal(9, "Blues"),
    na.value = "transparent",
    name = "Co-Release Percentage"
  ) +
  scale_x_discrete(position = "top") +
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 0, vjust = 0, size = 10, color = "black"),
    ...
    aes(
      label = ifelse(
        ...
        ifelse(value < 1,
               ...
               sprintf("%.0f", value)    # no decimals
        )),
      color = ifelse(value > max(cns_co_long$value, na.rm = TRUE) * 0.5, "white", "black")
    ),
    size = 3
  ) +
  scale_color_identity() +   # important so ggplot uses your colors directly
...
cardioresp_heat = ggplot(cardioresp_co_long, aes(x = chemical1, y = chemical2, fill = value)) +
  ...
  geom_text(
    aes(
      ...
      color = ifelse(
        value > max(cardioresp_co_long$value, na.rm = TRUE) * 0.5,
        "white", "black"
      )
    ),
    size = 3
  ) +
  scale_color_identity() +  # allows custom text colors
  scale_fill_gradientn(
    colors = brewer.pal(9, "Blues"),
    na.value = "transparent",
    name = "Co-Release Percentage"
...
devrep_heat = ggplot(devrep_co_long, aes(x = chemical1, y = chemical2, fill = value)) +
  ...
  geom_text(
    aes(
      ...
      color = ifelse(
        value > max(devrep_co_long$value, na.rm = TRUE) * 0.5,
        "white", "black"
      )
    ),
    size = 3
  ) +
  scale_color_identity() +  # allows us to apply color directly
  scale_fill_gradientn(
    colors = brewer.pal(9, "Blues"),
    na.value = "transparent",
    name = "Co-Release Percentage"
  ...
  geom_text(
    aes(
      label = ifelse(
        ...
        ifelse(
          value < 1,
          sprintf("%.1f", value),   # one decimal for <1
          sprintf("%.0f", value)    # no decimals for >=1
        )
      ),
      color = ifelse(
        value > max(lkt_co_long$value, na.rm = TRUE) * 0.5,  # threshold = 50% of max()
        "white", "black"
      )
    ),
    size = 3
  ) +
  scale_color_identity() +  # use custom text colors directly
  scale_fill_gradientn(
    colors = brewer.pal(9, "Blues"),
    na.value = "transparent",
    name = "Co-Release Percentage"
