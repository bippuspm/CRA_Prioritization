...
#Cardiorespiratory (Barium, cobalt, molybdenum, naphthalene, 2,5-Furandione (maleic anhydride), 1-hexadecanol, hydrogen fluoride)
```{r}
#total barium = 48
...
#hf + bar = 10
hf.bar = merge(hf, bar, by = 'trifd')
length(unique(hf.bar[["trifd"]]))

#hf + cob = 144
hf.cob = merge(hf, cob, by = 'trifd')
length(unique(hf.cob[["trifd"]]))

#hf + mol = 34
hf.mol = merge(hf, mol, by = 'trifd')
length(unique(hf.mol[["trifd"]]))

#hf + naph = 149
hf.naph = merge(hf, naph, by = 'trifd')
length(unique(hf.naph[["trifd"]]))

#hf + mbc = 9
hf.mbc = merge(hf, mbc, by = 'trifd')
length(unique(hf.mbc[["trifd"]]))

#hf + hex = 0
hf.hex = merge(hf, hex, by = 'trifd')
length(unique(hf.hex[["trifd"]]))
...
colnames(cardio) = c("Cobalt", "Molybdenum", "Naphthalene", "2,5-Furandione", "Hydrogen fluoride")
...
#CNS-carc = 0.0188765
#CNS-carc_CNS = 0.0119345
#CNS-cardio = 0.0347942
#CNS-edcs = 0.0644036
